namespace VokaTester.Features.StringSimilarity.Models.WeightedLevenshtein
{
    ...
    using static VokaTester.Features.StringSimilarity.Models.Levensthein.SimpleLevenstheinMethod;

    public class MyWeightedLevenshtein
    {
        private readonly ICharacterSubstitution characterSubstitution;
        private readonly ICharacterInsDel characterInsDel;
        ...
        public MyWeightedLevenshtein(ICharacterSubstitution characterSubstitution)
        ...
        public MyWeightedLevenshtein(
            ICharacterSubstitution characterSubstitution,
            ICharacterInsDel characterInsDel)
        {
            this.characterSubstitution = characterSubstitution;
            this.characterInsDel = characterInsDel;
        }
        ...
        public double Distance(string s1, string s2, double limit)
        {
            ...
            //Operations[,] nextOperation = new Operations[s1.Length + 1, s2.Length + 1];
            ...
            {
                char s1i = s1[i];
                double deletionCost = this.DeletionCost(s1i);
                ...
                for (int j = 0; j < s2.Length; j++)
                {
                    ...
                    double insertionCost = this.InsertionCost(s2j);
        ...
        private double InsertionCost(char c) => this.characterInsDel?.InsertionCost(c) ?? 1.0;

        private double DeletionCost(char c) => this.characterInsDel?.DeletionCost(c) ?? 1.0;
    }
}
